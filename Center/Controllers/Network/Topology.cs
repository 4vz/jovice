...
namespace Center
{
    ...
    {
        public static void Discovery(List<object> objects, TopologyDiscoveryTypes discovery)
        {
            ...
            string serviceType = (string)objects[9];
            ...
            {
                piMatch = Jovice.Database.Format("PI_SI = {0}", id);
                miMatch = Jovice.Database.Format("MI_SI = {0}", id);
                mcMatch = Jovice.Database.Format("MC_SI = {0}", id);
            }
            else if (discovery == TopologyDiscoveryTypes.PEInterface)
            {
                piMatch = Jovice.Database.Format("PI_ID = {0}", id);
            }
            else if (discovery == TopologyDiscoveryTypes.MEInterface)
            {
                miMatch = Jovice.Database.Format("MI_ID = {0}", id);
            }
...
order by n.NO_Active desc, a.PI_Status desc, a.PI_Protocol desc");
            ...
            //Quick.Log($"DISINI, {serviceType} {r.Count}");
...
", miMC);
                            if (r2.Count > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    r2 = Jovice.Database.Query("select NO_Name, NO_Manufacture, NO_Model, NO_Version, NO_TimeStamp from Node, MESDP where NO_IP = MS_IP and MS_ID = {0}", row2["MP_MS"].ToString());
...
", topologyLocalAccess);
            ...
            {
                #region OTHER SERVICES USING METRO
...
left join ServiceImmediate si on si.SI_ID = i.MI_SI
left join Service se on se.SE_ID = si.SI_SE
...
i2.MI_ID as MI2_ID, i2.MI_Description as MI2_Description, i2.MI_Status as MI2_Status, i2.MI_Protocol as MI2_Protocol,
n.NO_Name, n.NO_Manufacture, n.NO_Model, n.NO_Version, n.NO_TimeStamp,
c.MC_ID, c.MC_VCID, c.MC_Description as MC_Description, c.MC_MTU, c.MC_Status, c.MC_Protocol, c.MC_Type,
aw.AW_AG, se.SE_SID,
ni.NI_Name, nn.NN_Name
from 
Node n, Area ar, AreaWitel aw, MECircuit c
left join ServiceImmediate si on si.SI_ID = c.MC_SI
left join Service se on se.SE_ID = si.SI_SE
...
left join ServiceImmediate si on c.MC_SI = si.SI_ID
left join Service se on se.SE_ID = si.SI_SE
...
left join MEQOS qosb on a.MI_MQ_Output = qosb.MQ_ID
left join MEInterface b on a.MI_MI = b.MI_ID
left join ServiceImmediate si on a.MI_SI = si.SI_ID
left join Service se on se.SE_ID = si.SI_SE
left join NBInterface c on b.MI_TO_NI = c.NI_ID
left join NodeNeighbor d on c.NI_NN = d.NN_ID
where a.MI_MC = {0} and a.MI_ID <> {1}
...
", topologyLocalAccess);
            ...
            objects[9] = serviceType;

            if (objects[5] == null && detail != null)
            {
                objects[5] = detail;
            }

            objects.Add(serviceSubType); // 14
            ...
            objects.Add(topology.ToArray()); // 19
            objects.Add(vrf.ToArray()); // 20
            objects.Add(rateInput.ToArray()); // 21
            objects.Add(inputLimiter.ToArray()); // 22
            objects.Add(rateOutput.ToArray()); // 23
            objects.Add(outputLimiter.ToArray()); // 24
            objects.Add(topologyPurpose.ToArray()); // 25
            objects.Add(ip.ToArray()); // 26
            objects.Add(vcid.ToArray()); // 27
            objects.Add(nodeInfo.ToArray()); // 28
            objects.Add(localacc.ToArray()); // 29
            objects.Add(routeType.ToArray()); // 30
            objects.Add(ipd.ToArray()); // 31

            List<object> orders = new List<object>();

            string seID = (string)objects[11];

            if (seID != null)
            {
                string sid = (string)objects[4];
                ...
                foreach (Row2 ro in Jovice.Database.Query("select * from ServiceOrder where SO_SE = {0} order by SO_Created desc", seID))
                {
                    ...
                    orderData.Add(ro["SO_OID"].ToString());
                    orderData.Add(ro["SO_Created"].ToDateTime());
                    orderData.Add(ro["SO_Action"].ToString());
                    orderData.Add(ro["SO_Status"].ToString());
                    orderData.Add(ro["SO_AM"].ToString());

                    orders.Add(orderData.ToArray());
                }
            }
            else
            {
                ...
                {
                    // USING VRF
                    foreach (object vrfo in (object[])objects[20])
                    {
                        string dvrf = (string)vrfo;
 ...
 order by NameCount desc", dvrf);

                            if (vr > 0)
                            {
                                objects[6] = $"!{vr[0]["SC_Name"]}";
                                found = true;
                                break;
                            }
                        }
                ...
                if (!found)
                {
                    // USING CUSTOMER PREFIX FROM TENOSS
                    int sepid = vid.IndexOf('-');
                    if (sepid > -1)
                    {
                        string cid = vid.Substring(0, sepid);
            ...
            objects.Add(orders.ToArray());
        }
        
        public static void Prepare(SearchMatchResult matchResult)
        {
            matchResult.AddColumn("SubType");

            matchResult.AddColumn("Reserved15");
            matchResult.AddColumn("Reserved16");
            matchResult.AddColumn("Reserved17");
            matchResult.AddColumn("Reserved18");

            matchResult.AddColumn("Topology"); // 19
            matchResult.AddColumn("Vrf"); // 20
            matchResult.AddColumn("RateInput"); // 21
            matchResult.AddColumn("InputLimiter"); // 22
            matchResult.AddColumn("RateOutput"); // 23
            matchResult.AddColumn("OutputLimiter"); // 24
            matchResult.AddColumn("Purpose"); // 25
            matchResult.AddColumn("IP"); // 26
            matchResult.AddColumn("VCID"); // 27
            matchResult.AddColumn("NodeInfo"); // 28
            matchResult.AddColumn("LocalAccess"); // 29
            matchResult.AddColumn("RouteType"); // 30
            matchResult.AddColumn("IPD"); // 31
            matchResult.AddColumn("Orders"); // 32
        }
