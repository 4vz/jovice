...
(function () {
    ...
    (function (window) {
        ...
        center.init = function (p) {
            ...
            // login
            ogbox = ui.box(ctop)({ right: 15, width: 100, height: 40, cursor: "pointer", hide: true, click: function () { page.transfer("/signin"); } });
        ...
        center.icon = function (name) {
            ...
            else if (name == "lightning") return "M75.102,220l12.689-101.666H57.334L70.023,0h74.875L119.52,81.668h43.146L75.102,220z";
        };
        center.formatInterfaceName = function (name, man) {
            ...
            else if (man == "NEIGHBOR") {
                var splits = name.split(",");
                if (splits.length == 1) {
                    var namec = splits[0];
                    if (namec.startsWith("Ex")) name = namec.substr(2);
                    else name = namec;
                }
                else {
                    var splitsfix = [];
                    $.each(splits, function (is, vs) {
                        vs = vs.trim();
                        if (vs.startsWith("Ex")) {
                            splitsfix.push(vs.substr(2));
                        }
                        else splitsfix.push(vs);
                    });
                    name = "Aggregated Interface: " + splitsfix.join(", ") + "";
                }
            }
        ...
        center.showToolbar = function () {
            ctop.show();
            ui.marginTop(40);
        };
        center.hideToolbar = function () {
            ctop.hide();
            ui.marginTop(0);
        };
