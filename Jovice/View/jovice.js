(function () {
    ...
    (function (window) {
        ...
        jovice.init = function (p) {
            ...
            // ctop
            ctop = ui.box(ui.topContainer())({ color: 98, height: 46, width: "100%" });

            // ctop height
            ui.marginTop(46);

            nbox = ui.box(ctop)({ size: 46 });
            ui.icon(nbox, jovice.icon("logo"))({ size: 38, color: 75, position: [4, 4] });

            var aseaactive = false;
            asea = ui.box(ctop)({ color: 98, left: 46, height: 46, width: 400 });
            var aseasearch = ui.textinput(asea)({ font: 17, leftRight: [50, 0], top: 0, height: 46, design: false, hide: true });
            var aseaglass = ui.icon(asea, jovice.icon("glass"))({ size: [38, 38], color: 75, position: [4, 4] });
            var aseastart = ui.text(asea)({ noSelect: true, text: "Search anything here", font: ["body", 17], color: 65, position: [50, 12] });
            asea.enter(function (e) {
                if (!aseaactive) {
                    this.stop();
                    this.color(100, { duration: 166 });
                }
            });
            ...
            //vearch = ui.textinput(b_navigation)({ font: 17, width: null, leftRight: [50, 50], top: 13, hide: true, design: false });
            




            b_navigation = ui.box(ui.topContainer())({ color: 90, cursor: "text" });
